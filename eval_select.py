...
tf.app.flags.DEFINE_string('log', 'log.txt', '')
tf.app.flags.DEFINE_bool('no_write_images', False, 'do not write images')
tf.app.flags.DEFINE_bool('log_only', False, 'log only')
...
def analyze(path):
    with open(path) as f:
        content = f.readlines()
    content = [line for line in content if 'ckpt' in line or 'recall' in line]
    ckpts = [line for line in content if 'ckpt' in line]
    res = [line for line in content if 'recall' in line]
    res = [ast.literal_eval(r.strip('\n')[11:])['hmean'] for r in res]
    indexes = sorted(range(len(res)), key=lambda i: res[i], reverse=True)[:15]
    for i in indexes:
        print("top {}: {}, {}".format(i+1, ckpts[i], res[i]))
...
def get_undone():
    return [os.path.join(FLAGS.test_data_path, name) for name in undone]
